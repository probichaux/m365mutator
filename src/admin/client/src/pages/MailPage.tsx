...
type MailOp = 'send' | 'reply' | 'forward' | 'move';
const MAX_RUNS = 999;
...
export default function MailPage() {
  ...
  const doItNow = async () => {
    ...
    if (result.status === 200 && result.data.results) {
      ...
      showToast(
        d.failed === 0 ? tp('mail.mutate.toastOk', args) : tp('mail.mutate.toastFailures', args),
        d.failed === 0 ? 'success' : 'warning',
      );
    } else {
      showToast(result.data.error || tp('mail.mutate.toastFailed'), 'error');
    }
  };
        ...
        {run && (
          <div style={{ marginTop: 20 }}>
            <Text block weight="semibold" style={{ marginBottom: 8 }}>
              {tp('mail.mutate.summary', { runs: run.runs, total: run.totalActions, ok: run.ok, failed: run.failed })}
            </Text>
            {run.results.map((r, i) => (
            ...
            {run.truncated && (
              <Text size={200} block style={{ color: tokens.colorNeutralForeground3, marginTop: 6 }}>
                {tp('mail.mutate.truncated', { shown: run.results.length, total: run.totalActions })}
              </Text>
            )}
          </div>
        )}
