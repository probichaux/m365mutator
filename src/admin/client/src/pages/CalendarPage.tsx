...
type CalendarOp = 'meeting' | 'appointment';
const MAX_RUNS = 999;
...
export default function CalendarPage() {
  ...
  const doItNow = async () => {
    setRunning(true);
    setRun(null);
    const result = await api<CalendarRun>('POST', '/api/calendar/mutate', { runs });
    setRunning(false);
    ...
    if (result.status === 200 && result.data.results) {
      ...
      showToast(
        d.failed === 0 ? tp('calendar.mutate.toastOk', args) : tp('calendar.mutate.toastFailures', args),
        d.failed === 0 ? 'success' : 'warning',
      );
    } else {
      showToast(result.data.error || tp('calendar.mutate.toastFailed'), 'error');
    }
  };
    ...
    <div>
      ...
      <Card style={{ marginTop: 16 }}>
        ...
        <Text size={300} block style={{ color: tokens.colorNeutralForeground2, marginBottom: 16 }}>
          {tp('calendar.mutate.description')}
        </Text>
        ...
        {run && (
          <div style={{ marginTop: 20 }}>
            <Text block weight="semibold" style={{ marginBottom: 8 }}>
              {tp('calendar.mutate.summary', { runs: run.runs, total: run.totalActions, ok: run.ok, failed: run.failed })}
            </Text>
            {run.results.map((r, i) => (
            ...
            {run.truncated && (
              <Text size={200} block style={{ color: tokens.colorNeutralForeground3, marginTop: 6 }}>
                {tp('calendar.mutate.truncated', { shown: run.results.length, total: run.totalActions })}
              </Text>
            )}
          </div>
        )}
