...
type OneDriveOp = 'createText' | 'createDoc' | 'rename' | 'createFolder' | 'remove' | 'folderMove' | 'image';
const MAX_RUNS = 999;
...
export default function OneDrivePage() {
  ...
  const [run, setRun] = useState<OneDriveRun | null>(null);
  const [ready, setReady] = useState(false);

  const doItNow = async () => {
    setRunning(true);
    setRun(null);
    const result = await api<OneDriveRun>('POST', '/api/onedrive/mutate', { runs });
    setRunning(false);
    ...
    if (result.status === 200 && result.data.results) {
      ...
      showToast(
        d.failed === 0 ? tp('onedrive.mutate.toastOk', args) : tp('onedrive.mutate.toastFailures', args),
        d.failed === 0 ? 'success' : 'warning',
      );
    } else {
      showToast(result.data.error || tp('onedrive.mutate.toastFailed'), 'error');
    }
  };
  ...
  return (
    <div>
      ...
      <TargetPanel category="onedrive" onReadyChange={setReady} />

      <Card style={{ marginTop: 16 }}>
        <Text weight="semibold" size={400} block style={{ marginBottom: 4 }}>{tp('onedrive.mutate.title')}</Text>
        <Text size={300} block style={{ color: tokens.colorNeutralForeground2, marginBottom: 16 }}>
          {tp('onedrive.mutate.description')}
        </Text>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <RunStepper
            ...
            max={MAX_RUNS}
            disabled={running}
            ariaLabel={tp('onedrive.mutate.runsLabel')}
          />
          <Text size={200} style={{ color: tokens.colorNeutralForeground3 }}>{tp('onedrive.mutate.runsSuffix')}</Text>
          <Tooltip content={tp('run.needTargets')} relationship="label" visible={ready ? false : undefined}>
            ...
            >
              {running
                ? tp('onedrive.mutate.running')
                : tp('onedrive.mutate.doItNow') + (runs > 1 ? ` · ${runs}×` : '')}
            </Button>
          </Tooltip>
        </div>
        ...
        {run && (
          <div style={{ marginTop: 20 }}>
            <Text block weight="semibold" style={{ marginBottom: 8 }}>
              {tp('onedrive.mutate.summary', { runs: run.runs, total: run.totalActions, ok: run.ok, failed: run.failed })}
            </Text>
            {run.results.map((r, i) => (
            ...
            {run.truncated && (
              <Text size={200} block style={{ color: tokens.colorNeutralForeground3, marginTop: 6 }}>
                {tp('onedrive.mutate.truncated', { shown: run.results.length, total: run.totalActions })}
              </Text>
            )}
          </div>
        )}
